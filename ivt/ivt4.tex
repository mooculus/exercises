...
$[d,c]$ and $s$ is between
\wordChoice{\choice{$d$}\choice[correct]{$f(d)$}} and
\wordChoice{\choice{$c$}\choice[correct]{$f(c)$}}, then there is a
number \wordChoice{\choice[correct]{$b$}\choice{$f(b)$}} in
  \wordChoice{\choice{$[f(d),f(c)]$}\choice[correct]{$[d, c]$}} such that
  \wordChoice{\choice[correct]{$f(b) = s$}\choice{$f(s) = b$}}.
\begin{hint}
...
\begin{axis}[
            ...
            xticklabels={$d$,$b$,$c$}, yticklabels={$f(d)$,$s$,$f(c)$},
            axis on top,
          ]
