...
$[a,d]$ and $p$ is between
\wordChoice{\choice{$a$}\choice[correct]{$f(a)$}} and
\wordChoice{\choice{$d$}\choice[correct]{$f(d)$}}, then there is a
number \wordChoice{\choice[correct]{$r$}\choice{$f(r)$}} in
  \wordChoice{\choice{$[f(a),f(d)]$}\choice[correct]{$[a, d]$}} such that
  \wordChoice{\choice[correct]{$f(r) = p$}\choice{$f(p) = r$}}.
\begin{hint}
...
\begin{axis}[
            ...
            xticklabels={$a$,$r$,$d$}, yticklabels={$f(a)$,$p$,$f(d)$},
            axis on top,
          ]
