...
\item		$f$ is decreasing on $\answer{(1,\infty)}$, write DNE if no such interval(s) exist.
\item		$f$ is increasing on $\answer{(-\infty,1)}$, write DNE if no such interval(s) exist.
\item		At $x=1$, $f$ has
