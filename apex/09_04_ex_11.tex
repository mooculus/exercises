...
Given the graph of a function in rectangular (Cartesian) coordinates below, find the equation representing the graph in polar coordinates subject to the correct restriction(s) on $\theta$ and $r$.
 ...
 \begin{axis}[ymin=-0.1, ymax=2.1,
  ...
  \addplot[penColor, samples=400,domain=0:pi/2,
    data cs=polarrad,very thick] (\x,2);
 \end{axis} 
\end{tikzpicture}     
\end{center} 

\[
r(\theta)\begin{prompt}=\answer{2}\end{prompt}
\]
subject to the restriction(s) that
\[
0\le\theta\le\begin{prompt}\answer{\frac{\pi}{2}}\end{prompt}
\]
