...
\item		$f$ has a critical point(s) at $x = \answer{-1}$, write DNE if no such point(s) exist.
\item		$f$ is decreasing on $\answer{(-\infty,1)}$, write DNE if no such interval(s) exist.
\item		$f$ is increasing on $\answer{(-1,\infty)}$, write DNE if no such interval(s) exist.
\item		At $x=-1$, $f$ has
