...
\begin{enumerate}
\item		The domain of $f$ is $\answer{(2,\infty)}$.
\item		$f$ has a critical point(s) at $x = \answer{6}$, write DNE if no such point(s) exist.
\item		$f$ is decreasing on $\answer{(6,\infty)}$, write DNE if no such interval(s) exist.
\item		$f$ is increasing on $\answer{(2,6)}$, write DNE if no such interval(s) exist.
\item		At $x=6$, $f$ has 
