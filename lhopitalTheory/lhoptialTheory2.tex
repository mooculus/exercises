...
\[
\lim_{x \to \infty} \frac{x+\sin(x)}{\sqrt{x^2+1}}
\]
...
\begin{prompt}
	...
	\choice[correct]{l'H\^{o}pital's rule does not apply because $\lim_{x \to \infty} \frac{f'(x)}{g'(x)}$ does not exist.}
\end{multipleChoice}

If the limit does not exist, write $DNE$.

\[
\lim_{x \to \infty} \frac{x+\sin(x)}{\sqrt{x^2+1}} = \answer{1}
\]
\end{prompt}
