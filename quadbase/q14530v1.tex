\documentclass{ximera}
%\input{../preamble.tex}
\author{Emma Smith Zbarsky}
\license{Creative Commons Attribution 3.0 Unported}
\acknowledgement{https://quadbase.org/questions/q14530v1}
\begin{document}

\begin{exercise}

Find and identify the extrema of $q(r) = -7r+8$ on the domain
$1 \leq r \leq 5$.


\begin{hint}
This is a bounded extrema problem. First, check for critical points of
the function and then check the end points to determine whether they are
maxima or minima.
\end{hint}


\begin{hint}
Since $q= -7r+8$ is a linear function, the derivative $q' = -7$ is a
non-zero constant and thus has no critical points. Therefore, any
extrema are located at the ends of the domain. Since the slope $q'$ is
negative, that means that the left endpoint will be a maximum and the
right endpoint will be a minimum.

\begin{itemize}
\item
  $r=1$ is a maximum
\item
  $r=5$ is a minimum
\end{itemize}
\end{hint}


\begin{multipleChoice}
\choice[correct]{There are two extrema: one maximum at $r=1$ and one minimum at $r=5$.}
\choice{There are no extrema for the function $q$.}
\choice{There is one extremum: a minimum at $r=5$.}
\choice{There are two extrema: one minimum at $r=1$ and one maximum at $r=5$.}
\choice{There is one extremum: a minimum at $r=1$.}
\end{multipleChoice}

\end{exercise}
\end{document}
